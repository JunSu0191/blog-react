...
export function useAuth() {
  ...
  const initialUser = getStoredUser<User>();
  ...
  useEffect(() => {
    ...
    if (user) {
      queryClient.setQueryData(userApi.AUTH_ME_QUERY_KEY, user);
      setIsLoadingUser(false);
      return;
    }
  ...
  const updateUser = useCallback((nextUser: Partial<User> | User | null) => {
    setUser((previous) => {
      if (nextUser === null) {
        clearStoredUser();
        queryClient.removeQueries({ queryKey: userApi.AUTH_ME_QUERY_KEY });
      ...
      const resolved =
        previous ? { ...previous, ...nextUser } : (nextUser as User);
      setStoredUser(resolved);
      if (typeof resolved.id === "number") {
        setUserId(resolved.id);
      }
      queryClient.setQueryData(userApi.AUTH_ME_QUERY_KEY, resolved);
      return resolved;
    });
  }, [queryClient]);
