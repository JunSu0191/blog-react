...
export default function AdminRecentCommentsPreview() {
  const query = useAdminRecentComments();
          ...
          {query.data?.content.map((comment) => (
            ...
            >
              <div className="flex items-center gap-2">
                <StatusBadge hidden={Boolean(comment.deletedAt)} />
              ...
              <p className="mt-1 line-clamp-1 text-xs text-slate-500 dark:text-slate-400">
                {comment.authorName || comment.username || "알 수 없음"} ·{" "}
