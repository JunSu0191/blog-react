...
export default function ConversationList({
  ...
  if (isLoading) {
    return <div className="p-4 text-sm text-slate-500">대화방을 불러오는 중...</div>;
  }

  if (conversations.length === 0) {
    return <div className="p-4 text-sm text-slate-500">대화방이 없습니다.</div>;
  }
        ...
        return (
          ...
          >
            <div className="flex items-center gap-3">
              <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-black text-slate-600">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="shrink-0 text-[11px] text-slate-400">{formatTime(conversation.updatedAt)}</span>
                </div>
